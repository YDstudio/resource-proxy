...
We don't recommend using this resource proxy.
It is not being maintained and there's been no active development for many years.
There are many better solutions.

* Enable [CORS](http://enable-cors.org/server.html). Don't use non-CORS enabled web servers.
* Use [API keys](https://developers.arcgis.com/documentation/mapping-apis-and-services/security/api-keys/) to use services and access private content. See [API keys](https://developers.arcgis.com/documentation/mapping-apis-and-services/security/api-keys/) for more details.
